...
import { Form, FormGroup, FormControl, ControlLabel, Schema , ButtonToolbar, Button, Message, Notification} from 'rsuite';
  ...
  const handleSubmit = (data) =>{
    ...
    return (
        <div className="login_form_container">
          {(!props.email && state.loginDetails.is_current_user_logged_in) && <Link to="/user/organization">
              <Message
                  showIcon
                  type="info"
                  className="welcome_auth_message"
                  title="Login As"
                  description={`${state.userDetails.email}, Is this you?`}
              />
              </Link>
            }
