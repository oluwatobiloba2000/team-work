...
// import avatarSample from '../../img/samji_illustrator.jpeg';
import './accept_invite.css';
...
// if you want to accept invite, user needs to provide their invite key and email
function AcceptInvite(props) {
    ...
    const redirectToSignIn = () => {
        Notification["warning"]({
            ...
            description: 'You have to signin to join this organization'
        })
        setTimeout(() => {
            window.location.href = `${window.location.origin}/auth/login?entrypoint=invite&email=${urlEmail}&organizationID=${urlOrganizationID}&inviteKey=${urlInviteKey}&org=${urlOrgName}`
        }, 2000)
    }

    const acceptInvite = () => {
        if (state.userDetails && state.userDetails.email) {
                ...
                axios.put(`/org/invite/${urlOrganizationID}/accept?invitekey=${urlInviteKey}`)
                    .then(function (response) {
                        ...
                        if (response.status === 200) {
                            setTimeout(() => {
                                window.location.href = `${window.location.origin}/organization/${urlOrganizationID}`
                            }, 2000)
